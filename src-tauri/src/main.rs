...
fn app_data_dir(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    fs::create_dir_all(&dir).map_err(|error| format!("Could not create KitNote data directory: {error}"))?;
    Ok(dir)
}
...
fn copy_image_to_note(app: AppHandle, note_id: String, path: String) -> Result<CopiedImage, String> {
    ...
    fs::create_dir_all(&assets_dir).map_err(|error| format!("Could not create image asset directory: {error}"))?;
